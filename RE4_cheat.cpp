...
void PrintMemory( DWORD processID ){
	...
	while (VirtualQueryEx(hProcess, addr, &mbi, sizeof(mbi)))
	{
		if (mbi.State == MEM_COMMIT || mbi.Protect != PAGE_NOACCESS){
			...
			if (res != 0){
				for (INT64 i = 0; i < mbi.RegionSize; i++){
					if (buffer[i] == to_find[0]){
						for (int z = 1; z < Bytes_Case; z++){
							...
							if (z == (Bytes_Case - 1)){
								INT64 init_mem_items = i+z;
								INT64 addr_items = 0;
								addr += init_mem_items + 2;
								for (int a = 0; a < Number_items; a++){
									memcpy (&items[a], &buffer[init_mem_items+addr_items+2], sizeof(item_struct));
								...
								while(TRUE){
									for (int a = 0; a < Number_items; a++){
										if (items[a].item == 42){
											item_struct items_temp;
											if (!item_flag){
												addr += sizeof(item_struct) * a;
											}
											items[a].carga = 63952;
											WriteProcessMemory(hProcess, (void*)addr, &items[a], sizeof(item_struct), nullptr);
											ReadProcessMemory(hProcess, addr, &items_temp, sizeof(item_struct), nullptr);
											sleep(1);
											item_flag = TRUE;
										}
									}
